...
cutMatchS <- data.frame(lapply(cutMatchS, function(x) ifelse(x>50, 0, "S")), stringsAsFactors = F)
cutMatchE <- data.frame(lapply(cutMatchE, function(x) ifelse(x>50, 0, "E")), stringsAsFactors = F)
...
ggplot(numCuts) + geom_point(aes(x = Guide, y = Num, color = factor(Date)))
