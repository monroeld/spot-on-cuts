...
testing <- bind_rows(lapply(1:nrow(rois), function(x) {
...
inGenes <- filter(working, inRoi == 1)

# Match inRoi to genes.  Not working properly...
...
inGenes$both <- sapply(1:nrow(inGenes), function(x) {
  ...
  ifelse(inGene$Chr[x] == rois$Chr)
  max(rois$Gene[startint], rois$Gene[endint], na.rm = T)

start = which(inGenes$Chr_E[1] >= rois$Start)
end = which(inGenes$Chr_E[1] <= rois$End)
